...
def flip_images(left_img, right_img):
    # 对影像进行水平翻转
    flipped_left_img = torch.flip(right_img, [2])
    flipped_right_img = torch.flip(left_img, [2])
    return flipped_left_img, flipped_right_img
...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augmentor is not None:
            if self.sparse:
                img1, img2, label, flow, valid = self.augmentor(img1, img2, label, flow, valid)
            else:
                img1, img2, label, flow = self.augmentor(img1, img2, label, flow)
        ...
        if self.img_pad is not None:
            ...
            label = F.pad(label, [padW]*2 + [padH]*2)

        img1_1,img2_1=flip_images(img1,img2)
        label = p.encode_label_img(label)
        label = torch.from_numpy(label)

        flow = flow[:1]
        
        return img1, img2, img1_1, img2_1, flow, valid.float(), label
...
class KITTI_benchmark(StereoDataset):
    def __init__(self, aug_params=None, root='/home/mias/gftang/data/Instance/', image_set='testing'):
        super(KITTI_benchmark, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        ...
        disp_list = sorted(glob(os.path.join(root, image_set, 'disp_occ_0/*_10.png')))
        label_lst = sorted(glob(os.path.join(root, image_set, 'semantic_rgb/*_10.png')))

        for idx, (img1, img2, disp, label) in enumerate(zip(image1_list, image2_list, disp_list, label_lst)):
            self.image_list += [ [img1, img2] ]
            self.disparity_list += [ disp ]
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='/home/mias/gftang/data/myKITTI_ticoss', image_set='training'):
        ...
        if len(image1_list) == 0:
            image1_list = sorted(glob(os.path.join(root, 'image_2', image_set, '*_10.png')))
            image2_list = sorted(glob(os.path.join(root, 'image_3', image_set, '*_10.png')))
            disp_list = sorted(glob(os.path.join(root, 'disp_occ_0', image_set, '*_10.png')))
            label_lst = sorted(glob(os.path.join(root, 'semantic_rgb', image_set, '*_10.png')))
...
def fetch_dataloader(args):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name.startswith("middlebury_"):
            ...
            new_dataset = KITTI(aug_params, root=args.data_root)
